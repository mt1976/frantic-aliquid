...
func Count() (int, error) {
	...
	return activeDB.Count(&Behaviour_Store{})
}
...
func GetById(id any) (Behaviour_Store, error) {
	return GetBy(FIELD_ID, id)
}

func GetByKey(key any) (Behaviour_Store, error) {
	return GetBy(FIELD_Key, key)
}

func GetBy(field string, value any) (Behaviour_Store, error) {
	...
	if err := dao.IsValidFieldInStruct(field, Behaviour_Store{}); err != nil {
		return Behaviour_Store{}, err
	}

	record := Behaviour_Store{}
	logHandler.DatabaseLogger.Printf("Get %v where (%v=%v)", domain, field, value)

	if err := activeDB.Retrieve(field, value, &record); err != nil {
		clock.Stop(0)
		return Behaviour_Store{}, commonErrors.WrapRecordNotFoundError(domain, field, fmt.Sprintf("%v", value))
	}

	if err := record.PostGet(); err != nil {
		clock.Stop(0)
		return Behaviour_Store{}, commonErrors.WrapDAOReadError(domain, field, value, err)
	}
...
func GetAll() ([]Behaviour_Store, error) {

	dao.CheckDAOReadyState(domain, audit.GET, initialised) // Check the DAO has been initialised, Mandatory.

	recordList := []Behaviour_Store{}

	clock := timing.Start(domain, actions.GETALL.GetCode(), "ALL")

	if errG := activeDB.GetAll(&recordList); errG != nil {
		clock.Stop(0)
		return []Behaviour_Store{}, commonErrors.WrapNotFoundError(domain, errG)
	}
...
func GetAllWhere(field string, value any) ([]Behaviour_Store, error) {

	dao.CheckDAOReadyState(domain, audit.GET, initialised) // Check the DAO has been initialised, Mandatory.

	recordList := []Behaviour_Store{}
	resultList := []Behaviour_Store{}
...
func (record *Behaviour_Store) Update(ctx context.Context, note string) error {
...
func (record *Behaviour_Store) Spew() {
	logHandler.InfoLogger.Printf(" [%v] Record=[%+v]", strings.ToUpper(domain), record)
}

func (record *Behaviour_Store) Validate() error {
	return nil
}

func PostGet(recordList *[]Behaviour_Store) ([]Behaviour_Store, error) {
	clock := timing.Start(domain, actions.PROCESS.GetCode(), "POSTGET")
	returnList := []Behaviour_Store{}
	for _, record := range *recordList {
		if err := record.PostGet(); err != nil {
			return nil, err
		}
...
func (s *Behaviour_Store) PostGet() error {
...
func (record *Behaviour_Store) Export(name string) {
...
func Drop() error {
	return activeDB.Drop(Behaviour_Store{})
}
